...
const HotelDetails: React.FC<{
  ...
  const handleBuyNFT = async (tokenId: string) => {
    try {
      ...
      const network = await providerTestnet.getNetwork();
      const chainId = network.chainId;
      if (chainId !== BigInt(process.env.NEXT_PUBLIC_BNB_CHAIN_ID)) {
        throw new Error("Please connect to BNB Chain");
      }
      ...
      if (err.error.code === -32603) {
        // TODO: NFT is minted but it shows error, fix this
        setShowToast("success");
        setTimeout(() => router.reload(), 5000);
      } else {
        setShowToast("error");
        ...
        <div className="container mx-auto py-16">
          ...
          <section className="flex flex-col md:flex-row gap-10 mb-10">
            ...
            <div className="grid grid-cols-2 gap-2 flex-1">
              ...
              <Stats label="Last minted at" value="Sept 10, 2024" />
            </div>
          </section>
